...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--dir",
            required=True,
            help="Carpeta con archivos .xlsx, .xls o .csv",
        )
        parser.add_argument(
            "--tipo-default",
            default="reactivo",
    ...
    def handle(self, *args, **options):
        ...
        if not supported_files:
            raise CommandError("No se encontraron archivos .xlsx, .xls o .csv en la carpeta")
        ...
        for file_path in supported_files:
            ...
            venc_col = _guess_column(
                key_map,
                ["fecha vencimiento", "vencimiento", "fecha de vencimiento", "vence"],
            )
            tipo_col = _guess_column(key_map, ["tipo", "clase"])
            categoria_col = _guess_column(key_map, ["categoria", "familia", "grupo"])

            # If no explicit product name column exists, use first non-empty column.
            if not nombre_col:
                fallback_key = next(iter(key_map.values()), None)
                nombre_col = fallback_key
            ...
            # If the file does not contain category column, derive category from filename.
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"  OK -> creados: {local_created}, actualizados: {local_updated}, omitidos: {local_skipped}"
                )
            )
